...
class Validator extends BaseValidator {
    ...
    public static async addRole(req: Request, res: Response, next) {
        const validationChain = [
            body("roleName").isString().withMessage("Role's name must be a string"),
            body("roleName").notEmpty().withMessage("Role's name must not be empty"),
            body("roleName").isLength({ max: 50 }).withMessage("Role's name must be max 50 characters"),
            
            body("permissions").isArray().withMessage("Selected permissions must be in form of array"),
        ];
